...
public class CustomDatePicker implements View.OnClickListener, PickerView.OnSelectListener {
    ...
    public void show1(String dateStr) {
        if (!canShow() || TextUtils.isEmpty(dateStr)) return;

        // 弹窗时，考虑用户体验，不展示滚动动画
        if (setSelectedTime1(dateStr, false)) {
            mPickerDialog.show();
        }
    }
    ...
    public boolean setSelectedTime1(String dateStr, boolean showAnim) {
        return canShow() && !TextUtils.isEmpty(dateStr)
                && setSelectedTime1(DateFormatUtils.str2Long(dateStr, mCanShowPreciseTime), showAnim);
    }
    /**
     * 展示时间选择器
     *
    ...
    public boolean setSelectedTime1(long timestamp, boolean showAnim) {
        if (!canShow()) return false;
